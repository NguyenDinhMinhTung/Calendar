...
public class LessionDBHelper extends SQLiteOpenHelper {
    ...
    public boolean insert(TimeTableItem item) {
        return insert(item.getTimeTableID(), item.getTitle(), item.getStartDay());
    }
    ...
    public void delete(TimeTableItem item) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(TABLE_NAME, DB_ID + "= ?", new String[]{Integer.toString(item.getTimeTableID())});
        db.close();
    }
    ...
    public int getNewId() {
        ...
        if (!res.isAfterLast()) {
            id = res.getInt(res.getColumnIndex(DB_ID));
        }
        return id + 1;
    }
}
